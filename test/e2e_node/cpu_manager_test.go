...
func enableCPUManagerInKubelet(f *framework.Framework, cleanStateFile bool) (oldCfg *kubeletconfig.KubeletConfiguration) {
	return configureCPUManagerInKubelet(f, cleanStateFile, cpuset.CPUSet{}, false, []string{})
}

func configureCPUManagerInKubelet(f *framework.Framework, cleanStateFile bool, reservedSystemCPUs cpuset.CPUSet, enableOptions bool, options []string) (oldCfg *kubeletconfig.KubeletConfiguration) {
...
func runCPUManagerTests(f *framework.Framework) {
	...
	ginkgo.It("should assign CPUs as expected based on strict SMT alignment", func() {
		smtAlignOpt := fmt.Sprintf("option=%s", cpumanager.RejectNonSMTAlignedOption)
		_, cpuAlloc, _ = getLocalNodeCPUDetails(f)
		smtLevel := getSMTLevel()

		// strict SMT alignment is trivially verified and granted on non-SMT systems
		if smtLevel < 2 {
			e2eskipper.Skipf("Skipping CPU Manager %s tests since SMT disabled", smtAlignOpt)
		}

		// our tests want to allocate a full core, so we need at last 2*2=4 virtual cpus
		if cpuAlloc < int64(smtLevel*2) {
			e2eskipper.Skipf("Skipping CPU Manager %s tests since the CPU capacity < 4", smtAlignOpt)
		}
		...
		oldCfg = configureCPUManagerInKubelet(f, cleanStateFile, cpuset.NewCPUSet(0), true, []string{cpumanager.RejectNonSMTAlignedOption})
	...
	ginkgo.It("should ignore CPU policy options with feature gate disabled", func() {
		smtAlignOpt := fmt.Sprintf("option=%s", cpumanager.RejectNonSMTAlignedOption)
		_, cpuAlloc, _ = getLocalNodeCPUDetails(f)
		smtLevel := getSMTLevel()
		...
		// TODO: we assume the first available CPUID is 0, which is pretty fair, but we should probably
		// check what we do have in the node.
		oldCfg = configureCPUManagerInKubelet(f, cleanStateFile, cpuset.NewCPUSet(0), false, []string{cpumanager.RejectNonSMTAlignedOption})
