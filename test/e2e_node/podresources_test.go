...
func podresourcesGetAllocatableResourcesTests(f *framework.Framework, cli kubeletpodresourcesv1alpha1.PodResourcesListerClient, sd *sriovData) {
	ginkgo.By("checking the devices known to the kubelet")
	resp, err := cli.GetAllocatableResources(context.TODO(), &kubeletpodresourcesv1alpha1.AllocatableResourcesRequest{})
	framework.ExpectNoError(err)
