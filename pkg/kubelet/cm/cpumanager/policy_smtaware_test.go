...
func TestSMTAwarePolicyName(t *testing.T) {
	policy := &smtAwarePolicy{}

	policyName := policy.Name()
	if policyName != "smtaware" {
		t.Errorf("SMTAwarePolicy Name() error. expected: none, returned: %v", policyName)
	}
}

func TestSMTAwarePolicyAdmit(t *testing.T) {
	testPolicy, _ := NewSMTAwarePolicy(
		topoSingleSocketHT,
		1,
		cpuset.NewCPUSet(0),
	...
	testCases := []struct {
		...
		{
			description: "request even cpus - admit",
			pod:         makePod("fakePod", "fakeContainer", "2", "2"),
			admit:       true,
		},
		{
			description: "request uneven cpus - not admit",
...
// non-regression with static policy follows

func TestSMTAwarePolicyStart(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			p, _ := NewSMTAwarePolicy(testCase.topo, testCase.numReservedCPUs, cpuset.NewCPUSet(), topologymanager.NewFakeManager())
			policy := p.(*smtAwarePolicy)
			st := &mockState{
			...
			if !reflect.DeepEqual(err, testCase.expErr) {
				t.Errorf("SMTAwarePolicy Start() error (%v). expected error: %v but got: %v",
			...
			if !testCase.stDefaultCPUSet.IsEmpty() {
				for cpuid := 1; cpuid < policy.topology.NumCPUs; cpuid++ {
					if !st.defaultCPUSet.Contains(cpuid) {
						t.Errorf("SMTAwarePolicy Start() error. expected cpuid %d to be present in defaultCPUSet", cpuid)
					}
				}
			}
...
func TestSMTAwarePolicyAdd(t *testing.T) {
	...
	for _, testCase := range testCases {
		policy, _ := NewSMTAwarePolicy(testCase.topo, testCase.numReservedCPUs, cpuset.NewCPUSet(), topologymanager.NewFakeManager())
		...
		if !reflect.DeepEqual(err, testCase.expErr) {
			t.Errorf("SMTAwarePolicy Allocate() error (%v). expected add error: %v but got: %v",
				testCase.description, testCase.expErr, err)
		}
		...
		if testCase.expCPUAlloc {
			...
			if !found {
				t.Errorf("SMTAwarePolicy Allocate() error (%v). expected container %v to be present in assignments %v",
					testCase.description, container.Name, st.assignments)
			}

			if !reflect.DeepEqual(cset, testCase.expCSet) {
				t.Errorf("SMTAwarePolicy Allocate() error (%v). expected cpuset %v but got %v",
					testCase.description, testCase.expCSet, cset)
			}

			if !cset.Intersection(st.defaultCPUSet).IsEmpty() {
				t.Errorf("SMTAwarePolicy Allocate() error (%v). expected cpuset %v to be disoint from the shared cpuset %v",
		...
		if !testCase.expCPUAlloc {
			...
			if found {
				t.Errorf("SMTAwarePolicy Allocate() error (%v). Did not expect container %v to be present in assignments %v",
					testCase.description, container.Name, st.assignments)
			}
		}
	}
...
func TestSMTAwarePolicyRemove(t *testing.T) {
	testCases := []staticPolicyTest{
		{
	...
	for _, testCase := range testCases {
		...
		if !reflect.DeepEqual(st.defaultCPUSet, testCase.expCSet) {
			t.Errorf("SMTAwarePolicy RemoveContainer() error (%v). expected default cpuset %v but got %v",
		...
		if _, found := st.assignments[testCase.podUID][testCase.containerName]; found {
			t.Errorf("SMTAwarePolicy RemoveContainer() error (%v). expected (pod %v, container %v) not be in assignments %v",
				testCase.description, testCase.podUID, testCase.containerName, st.assignments)
		}
	}
}

func TestSMTAwareTopologyAwareAllocateCPUs(t *testing.T) {
	testCases := []struct {
		description     string
	...
	for _, tc := range testCases {
		p, _ := NewSMTAwarePolicy(tc.topo, 0, cpuset.NewCPUSet(), topologymanager.NewFakeManager())
		policy := p.(*smtAwarePolicy)
		st := &mockState{
		...
		err := policy.Start(st)
		if err != nil {
			t.Errorf("SMTAwarePolicy Start() error (%v)", err)
			continue
		}

		cset, err := policy.allocateCPUs(st, tc.numRequested, tc.socketMask, cpuset.NewCPUSet())
		if err != nil {
			t.Errorf("SMTAwarePolicy allocateCPUs() error (%v). expected CPUSet %v not error %v",
				tc.description, tc.expCSet, err)
			continue
		}

		if !reflect.DeepEqual(tc.expCSet, cset) {
			t.Errorf("SMTAwarePolicy allocateCPUs() error (%v). expected CPUSet %v but got %v",
...
type smtAwarePolicyTestWithResvList struct {
	description     string
	topo            *topology.CPUTopology
	numReservedCPUs int
...
func TestSMTAwarePolicyStartWithResvList(t *testing.T) {
	testCases := []smtAwarePolicyTestWithResvList{
		{
			description:     "empty cpuset",
			topo:            topoDualSocketHT,
			numReservedCPUs: 2,
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			p, err := NewSMTAwarePolicy(testCase.topo, testCase.numReservedCPUs, testCase.reserved, topologymanager.NewFakeManager())
...
func TestSMTAwarePolicyAddWithResvList(t *testing.T) {
	...
	for _, testCase := range testCases {
		policy, _ := NewSMTAwarePolicy(testCase.topo, testCase.numReservedCPUs, testCase.reserved, topologymanager.NewFakeManager())
